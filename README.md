...
| `block-report.js` | `buildBlockReportMarkdown(catalogue, blockNumber)` (plans + performance + feedback + raw JSON, for the block-finished export) and `extractTuningGuidance(md)` (pulls the auto-applied LLM-guidance section out of a returned tuning file) |
...
- Tabs: **Today** (greeting + phase block-progress ring **with current-block figure `blocks_done.sessions_this_block`, e.g. `1.2`** + metric tiles w/ sparklines + **equipment checkboxes** (paddles/pull buoy/bars/rings/weights — persisted per device, passed to generation) + session), **Log** (Planned vs External; pool CSV + plan-file; dryland per-set boxes; feedback + "completed fully"; **on a log that completes a block → "Block finished" card with a `.md` analysis export**), **Feedback** (LLM debrief of last session), **Graphs** (self-hosted Chart.js line charts + refresh; trend visuals — Graphs + Today sparklines — hide known-bad early data via `trendSeries`/`TREND_AFTER_DATE` in app.js, currently excluding dates ≤ 2026-04-10; display-only, catalogue untouched), **History** (rolling bests + session list), **Settings** (Gemini key/model + **GitHub sync** config/test/pull/push + **Coaching tuning** (paste a returned tuning .md → its "LLM guidance" section stored in `catalogue.llm_tuning`, synced, injected into generation + feedback prompts) — all stored in localStorage; Gemini shared real+demo, sync real-only).

### Coaching feedback loop
The app can be reviewed + improved by an external Claude project: log a block → **download its analysis export** (`buildBlockReportMarkdown`: prescribed plans + actual performance + feedback + raw JSON) → paste it into a Claude.ai project primed by **`docs/coaching-project-brief.md`** → that project returns a **tuning .md** → paste it into **Settings → Coaching tuning**. The `## LLM guidance (auto-applied)` section is auto-injected into the generation + analysis prompts (and syncs via the catalogue); the `## Deterministic core changes` section is a **dev-implemented spec** (NOT auto-applied — keeps the deterministic core a trusted gate). Logged app-generated sessions now store their prescribed `plan` so the export can compare prescribed-vs-actual. Planned future slice: a "training camp" self-play harness (auto-generate → synthetic results → feedback → review).
...
## Done
Engine (all modules, 202 tests). Web app: Today/Log/Feedback/Graphs/History/Settings. Phases 1–3 block-based. Demo. Live Gemini verified. **GitHub sync** (pull/push catalogue to a private repo; auto-pull on open; auto-push; conflict resolution). **PWA / offline** (manifest + maskable icon; root-scoped service worker; self-hosted Inter + Chart.js). **DEPLOYED & LIVE** at `https://gozibbygo.github.io/swim-coach/` (GitHub Pages; catalogue sync repo `GoZibbyGo/swim-catalogue`). **Equipment checkboxes** → generation. **Current-block `1.2` figure**, **block-finished analysis export**, and the **coaching feedback loop** (export → `docs/coaching-project-brief.md` → tuning .md → Settings → auto-applied LLM guidance). SW cache currently `swimcoach-v4` (bump on every shell change).
