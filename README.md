...
## Done
Engine (all modules, 198 tests). Web app: Today/Log/Feedback/Graphs/History/Settings. Phases 1–3 block-based. Demo. Live Gemini verified. **GitHub sync** (pull/push catalogue to a private repo; auto-pull on open; auto-push; conflict resolution). **PWA / offline** (manifest + maskable icon; root-scoped service worker; self-hosted Inter + Chart.js). **DEPLOYED & LIVE** at `https://gozibbygo.github.io/swim-coach/` (GitHub Pages; catalogue sync repo `GoZibbyGo/swim-catalogue`). **Equipment checkboxes** → generation. **Current-block `1.2` figure**. **Coaching feedback loop** — block-finished export + `scripts/eval-batch.js` self-play, both → a Claude.ai project (briefs in `docs/`) → a feedback file → **code changes by Claude Code** (no in-app runtime tuning; that was removed). SW cache currently `swimcoach-v7` (bump on every shell change). Coaching improvements arrive as **code changes from a grading review** (round 1 applied: empty-plan rejection, dryland→library, effort-not-pace targets, fixed progress endpoints, cross-block dryland spacing, sprint-quality + anti-fabrication-feedback rules).
