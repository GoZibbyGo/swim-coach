...
- Tabs: **Today** (greeting + phase block-progress ring + metric tiles w/ sparklines + session), **Log** (Planned vs External; pool CSV + plan-file; dryland per-set boxes; feedback + "completed fully"), **Feedback** (LLM debrief of last session), **Graphs** (self-hosted Chart.js line charts + refresh), **History** (rolling bests + session list), **Settings** (Gemini key/model + **GitHub sync** config/test/pull/push — all stored in localStorage; Gemini shared real+demo, sync real-only).
