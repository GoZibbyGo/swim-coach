# Swim Coach — session handoff (current state: 2026-05-29, SW cache v14)
...
- **217 tests** pass (`npm test`). **SW cache currently `swimcoach-v14`** (bump on every precached-file change).
- `.env` (gitignored, app root) holds the real `GEMINI_API_KEY` for eval re-runs. Never put the key in chat/commits.

## 2. How to run
```
npm test                                   # 217 tests (Node built-in runner)
...
2. **Bump `CACHE` in `sw.js`** (v14 → v15 …) whenever a precached file changes, AND add any *new* precached file to the `SHELL` array in `sw.js`.
...
- **Ad-hoc fix (2026-05-29, v14):** user reported "session 2 of block 3" when expecting "session 1 of block 3." The 28/05 session had been logged twice — once as the planned technique session (id 20, app_generated, closed Block 2) and once as External (id 21, inferred "mixed", incremented Block 3's pool count). Same date, same byte-identical `breakdown`. Fixed with (a) one-time gated migration `dedupe_identical_sessions_v1` that removes any session whose `(date, type, breakdown signature)` matches an earlier kept one and rolls back the block counters by the removed sessions' types; (b) a **log-time duplicate guard** in `app.js` — before `logSession`, scans existing sessions for the same `(date, type)` and (for pool) a near-identical `distance_m`, then `confirm()`s before logging a second copy. Belt-and-suspenders: the migration cleans up history, the guard prevents recurrence.
...
- Everything above is implemented, tested, committed, and **live at v14**.
