...
export function buildPrompt(decision, catalogue, targets, opts = {}) {
  ...
  const systemPrompt = [
    ...
    '- SAY EACH DESCRIPTOR ONCE. A set has `drill`, `equipment`, `effort` and `breathing` fields, and they are joined into ONE line the athlete reads. Do not restate the same instruction in two of them — `drill: "25m Fingertip Drag / 25m Fast Free"` with `effort: "25m drill / 25m fast"` renders as "8x50m 25m Fingertip Drag / 25m Fast Free, 25m drill / 25m fast", which reads as two different things to do. Put the length-by-length structure in `drill` and use `effort` only for information the drill does NOT already give (an RPE, a pace target, "build 70-100%").',
